 ...
 * @property { boolean } _main_canvas.fullscreen - Main canvas fullscreen toggle.
 *
 * @property { number } _current_frames_per_second - Frames per second.
 ...
 *
 * @property {number} _update_distance - Distance from player object to update objects.
 * @property {number} _render_distance - Distance from player object to render objects.
 *
...
export class Core {
    constructor() {
        ...
        this._frame_record = [];
        this._average_frames_per_second = 0;
        ...
        this._update_distance = 1500;
        this._render_distance = 3000;
    ...
    _import_game = (game) => {
        ...
        if (!game.objects) {
            console.error(
                "[s2d-engine: _import_game] Game object is missing 'objects' property."
            );
        }
        ...
        if (game.internal_objects) {
            game.internal_objects.map((internal_object_name) => {
                switch (internal_object_name) {
                    ...
                    default:
                        console.warn(
                            `[s2d-engine: _import_game] Internal object '${internal_object_name}' not found.`
                        );
                        break;
                }
            });
    ...
    _destroy_object = (identifier) => {
        ...
        if (!object) {
            console.warn(
                `[s2d-engine: _destroy_object] Object with identifier '${identifier}' not found.`
            );
        } else {
    ...
    _parse_object = (raw_object) => {
        ...
        if (raw_object.flags) {
            raw_object.flags.forEach((flag) => {
                ...
                if (flag == "IS_PLAYER")
                    this._player_object_identifier = raw_object.identifier;
            });
        }
        ...
        if (raw_object.sprite) {
            ...
            parsed_object.sprite = {
                ...
                render_width:
                    raw_object.sprite.render_width || raw_object.sprite.width,
                render_height:
                    raw_object.sprite.render_height || raw_object.sprite.height,
            };
        ...
        if (raw_object.collision_box) {
            parsed_object.collision_box =
                Vector4D.from_x_and_y_and_width_and_height(
                    raw_object.collision_box.x,
                    raw_object.collision_box.y,
                    raw_object.collision_box.width,
                    raw_object.collision_box.height
                );
        }
        ...
        if (raw_object.global_position)
            parsed_object.global_position = Vector2D.from_x_and_y(
                raw_object.global_position.x,
                raw_object.global_position.y
            );
        ...
        // Parse spawn_tile
        if (raw_object.spawn_tile)
            parsed_object.spawn_tile = raw_object.spawn_tile;

        // Parse render layer
        if (raw_object.render_layer)
            parsed_object.render_layer = raw_object.render_layer;
        ...
        if (raw_object.actions) {
            ...
            if (!input_manager) {
                console.warn(
                    "[s2d-engine: _parse_object] Input manager not found, but actions have been specified."
                );
            }
        ...
        // Parse unparsed
        Object.getOwnPropertyNames(raw_object).forEach((property) => {
            if (!parsed_object[property]) {
                parsed_object[property] = raw_object[property];
            }
        });
    ...
    _update_camera_position = () => {
        const player_object = this._get_object_by_identifier(
            this._player_object_identifier
        );
        this._camera_position = player_object.global_position.subtract(
            this._canvas_center
        );
    };
    ...
    _loop = () => {
        ...
        // Calculate frames per second over the last 100 frames
        this._current_frames_per_second = 1 / this._delta_time;
        this._frame_record.push(this._current_frames_per_second);
        if (this._frame_record.length > 100) {
            this._frame_record.shift();
        }
        let total_frametime = 0;
        this._frame_record.forEach(
            (frametime) => (total_frametime += frametime)
        );
        this._average_frames_per_second =
            total_frametime / this._frame_record.length;

        // Only run if game is in focus
        if (!this._out_of_focus) {
            // If tilemap is being used, dont run game until tilemap is loaded
            const tilemap_manager = this._get_object_by_identifier(
                "INTERNAL_tilemap_manager"
            );
            if (tilemap_manager) {
            ...
            // Clear canvas
            this._main_canvas.context.clearRect(
                0,
                0,
                this._main_canvas.element.width,
                this._main_canvas.element.height
            );
            ...
            Object.keys(this._objects).forEach((object_key, index) => {
                ...
                if (object.global_position) {
                    ...
                    const dist = object.global_position.distance(
                        player_object.global_position
                    );
                    if (dist < this._update_distance) {
                        update_object = true;
                    }
                    if (dist < this._render_distance) {
                        render_object = true;
                    }
                }
                // Add object to update/render queue
                if (update_object) {
                    this._add_to_update_queue(object, object.update);
                }
                if (render_object) {
                    this._add_to_render_queue(object, object.render);
                }
            });
    ...
    _update_objects = (delta) => {
        ...
        this._update_queue.map((update_item, index) => {
            ...
            if (update_item.self.spawn_tile && !update_item.self.spawned) {
                const spawn_position = this._get_spawn_position(
                    update_item.self.spawn_tile
                );
                update_item.self.global_position = Vector2D.from_x_and_y(
                    spawn_position.x,
                    spawn_position.y
                );
    ...
    _get_spawn_position = (tile_identifier) => {
        ...
        const tiles = tilemap_manager.get_tiles(
            tilemap_manager,
            tile_identifier
        );
        let spawn_position = {
        ...
        if (!tiles) {
            console.warn(
                `[s2d-engine: _update_objects] Spawn tile '${tile_identifier}' not found!`
            );
        } else {
        ...
        spawn_position = Vector2D.from_x_and_y(
            spawn_position.x,
            spawn_position.y
        );
    ...
    _render_objects = () => {
        ...
        this._render_queue.map((render_item, index) => {
            let position = render_item.self.global_position;

            // Calculate screen position for non UI objects
            if (!render_item.self.flags.IS_UI) {
                position = this._global_to_screen(
                    render_item.self.global_position
                ).floor();
            }

            // Check if object uses sprite and if so, render sprite
            if (
                render_item.self.flags.USE_SPRITE &&
                render_item.self.sprite.ready
            ) {
                const sprite = render_item.self.sprite;
                const sprite_position = position.subtract(
                    Vector2D.from_x_and_y(
                        sprite.render_width / 2,
                        sprite.render_height / 2
                    )
                );

                // If object specifies sprite angle, rotate sprite
                // if (render_item.self.sprite.angle) {
                let angle = render_item.self.rotation || 0;

                if (render_item.self.sprite_angle)
                    angle = render_item.self.sprite_angle;

                this._main_canvas.context.save();
                this._main_canvas.context.translate(position.x, position.y);
                this._main_canvas.context.rotate(angle);
                this._main_canvas.context.drawImage(
                    sprite.image,
                    -sprite.render_width / 2,
                    -sprite.render_height / 2,
                    sprite.render_width,
                    sprite.render_height
                );
            ...
            if (render_item.self.render) {
                render_item.self.render(
                    this,
                    render_item.self,
                    this._main_canvas.context,
                    position
                );
            }
            ...
            if (
                this.flags.RENDER_COLLISION_BOXES &&
                render_item.self.collision_box
            ) {
                const collision_box = render_item.self.collision_box;
                this._main_canvas.context.fillStyle = "rgba(0, 0, 255, 0.3)";
                this._main_canvas.context.fillRect(
                    position.x + collision_box.x,
                    position.y + collision_box.y,
                    collision_box.width,
